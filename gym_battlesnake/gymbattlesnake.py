...
env_obsptr = wrap_function(gamelib, 'env_getobspointer', ctypes.POINTER(ctypes.c_ubyte),
                           [ctypes.c_void_p, ctypes.c_uint])
env_actptr = wrap_function(gamelib, 'env_getactpointer', ctypes.POINTER(ctypes.c_ubyte),
                           [ctypes.c_void_p, ctypes.c_uint])
...
class BattlesnakeEnv(VecEnv):
    ...
    def __init__(self, n_threads=4, n_envs=16, opponents=[]):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(LAYER_WIDTH, LAYER_HEIGHT, NUM_LAYERS),
                                            dtype=np.uint8)
    ...
    def step_wait(self):
        ...
        for i in range(self.n_envs):
            if infoptr[i].ate:
                #  more reward if ate when health is low
                rews[i] += (101 - infoptr[i].health) / 100
            if infoptr[i].over:
                dones[i] = True
                info[i]['episode'] = {}
                if infoptr[i].alive:
                    # reward based on how long he survived before game over
                    # TODO remove this , no logical profit
                    rews[i] += int(infoptr[i].turn / 100)
                    info[i]['episode']['r'] = int(infoptr[i].turn / 100)
                else:
                    # TODO if killed by player -1
                    # TODO if killed by self body -2
                    # TODO if killed by wall -3
                    rews[i] -= 2.0
                    info[i]['episode']['r'] = -2.0
                info[i]['episode']['l'] = infoptr[i].turn
            else:
                # reward for being alive each step
                if infoptr[i].alive:
                    rews[i] += 0.01
